        ...
        <nav style={{ flex:1, padding:'8px 0' }}>
          {NAV.map((item, i) => {
            if ('section' in item) return (
              <div key={i} style={{ fontSize:8, letterSpacing:3, color:'rgba(168,144,112,0.5)', textTransform:'uppercase', padding:'14px 18px 5px', fontFamily:'Georgia,serif' }}>{item.section}</div>
            )
            const active = item.exact ? p === item.href : p.startsWith(item.href!)
            return (
              <Link key={item.href} href={item.href!} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 18px', textDecoration:'none', background:active?'rgba(200,134,10,0.12)':'transparent', borderLeft:active?'2px solid #C8860A':'2px solid transparent', color:active?'#E8B84B':'#A89070', fontSize:12, fontFamily:'Georgia,serif', letterSpacing:0.5, transition:'all 0.15s' }}>
                <span style={{ fontSize:13 }}>{item.icon}</span>{item.label}
              </Link>
            )
          })}
        </nav>
        <div style={{ padding:'12px 18px', borderTop:'1px solid rgba(200,134,10,0.15)' }}>
          <Link href="/" style={{ display:'flex', alignItems:'center', gap:8, textDecoration:'none', color:'#A89070', fontSize:11, fontFamily:'Georgia,serif' }}>← Ver tienda</Link>
        </div>
      </aside>
